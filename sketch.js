...
function preload() {
  backgroundImg = loadImage("assets/background copy.gif")
}
  ...
  for (var i = 0; i < computerArrows.length; i++) {
    ...
    if (
      ...
      if (playerArcherLife == 0) {
        player.collapse = false
        Matter.Body.setStatic(playerArcher.body,false)
        Matter.Body.setStatic(player.body,false)
        Matter.Body.setPosition(player.body,{x: 140,y: player.body.position.y})
      } 
    }
  }
}
